...
function Header ({ setView, projectsInvited, setModalOpen }) {

  let display;

  if(projectsInvited.length > 0) {
    display = <button onClick={() => {document.getElementById("invitationPopUp").showModal(); setModalOpen(true)}} className="bg-[#1da1f2] hover:bg-emerald-500 text-white font-bold py-2 px-4 border border-red-700 rounded-[40%/10%]">Invitations ({projectsInvited.length}) </button>
  }
